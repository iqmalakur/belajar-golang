...
func TestValidationErrors(t *testing.T) {
	...
	if err != nil {
		...
		for _, fieldError := range validationErrors {
			fmt.Println("error", fieldError.Field(), "on tag", fieldError.Tag(), "with error", validationErrors.Error())
		}
	}
}
...
func TestMap(t *testing.T) {
	...
	type User struct {
		...
		Addresses []Address `validate:"required,dive"`
		Hobbies   []string  `validate:"dive,required,min=3"`
		// untuk validasi key harus diawali keys dan diakhiri endkeys
...
func MustValidUsername(field validator.FieldLevel) bool {
	// field.Field().String() // bisa juga seperti ini
